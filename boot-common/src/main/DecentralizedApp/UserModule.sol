 ...
 *            UserModule本身也是一个合约，它需要部署到链上；同时，它又负责管理用户的合约。只有添加到模块中的用户合约，用户才能在dapp中调用这些合约
 */
...
contract UserModule is BaseModule {
    ...
    function UserModule() {
        ...
        //定义模块合约名称
        string memory moduleName = "UserModule";

        //定义模块合约名称
        string memory moduleDesc = "用户模块";
        ...
        string memory moduleDescription = moduleName.concat(moduleVersion, "-User-DAPP");

        //把合约注册到JUICE链上
        LibLog.log("register UserModule");
        ...
        tmpModule.moduleUrl = "http://202.114.114.47:30001/PoL/";
        ...
        LibLog.log("publish UserModule , exec insert ...");
        ret = addModule(tmpModule.toJson());
        moduleId = ret.recoveryToString();

        //添加用户合约到模块合约中
        LibLog.log("add UserManager to UserModule");
        ret = initContract(moduleName, moduleVersion, "UserManager", "验证合约", "0.0.1.0");
        if (ret != 0) {
            LibLog.log("add UserManager to UserModule failed");
            return;
        }


        //返回消息，以便控制台能看到是否部署成功
        Notify(0, "deploy UserModule success");
    }
